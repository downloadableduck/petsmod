...
public class ClientPhantomModel extends HierarchicalModel<ClientPhantom> {
    ...
    public ClientPhantomModel(ModelPart modelPart) {
        this.root = modelPart;
    ...
    public void setupAnim(ClientPhantom phantom, float f, float g, float h, float i, float j) {
        float k = ((float) (phantom.getId()) * 3 + h) * 7.448451F * ((float) Math.PI / 180F);
        float l = 16.0F;
        this.leftWingBase.zRot = Mth.cos(k) * 16.0F * ((float) Math.PI / 180F);
        this.leftWingTip.zRot = Mth.cos(k) * 16.0F * ((float) Math.PI / 180F);
        this.rightWingBase.zRot = -this.leftWingBase.zRot;
        this.rightWingTip.zRot = -this.leftWingTip.zRot;
        this.tailBase.xRot = -(5.0F + Mth.cos(k * 2.0F) * 5.0F) * ((float) Math.PI / 180F);
        this.tailTip.xRot = -(5.0F + Mth.cos(k * 2.0F) * 5.0F) * ((float) Math.PI / 180F);
    }
}
