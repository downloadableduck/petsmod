...
public class ClientZombieVillagerProfessionLayer extends RenderLayer<@NotNull ClientZombieVillager, ClientZombieVillagerModel> {
    ...
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource source, int i, ClientZombieVillager entityRenderState, float f, float g, float h, float k, float l, float u) {
        ...
        switch (CONFIG.zombieVillagerSkin) {
            case "armorer" ->
                    renderColoredCutoutModel(this.getParentModel(), ARMORER_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "butcher" ->
                    renderColoredCutoutModel(this.getParentModel(), BUTCHER_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "cartographer" ->
                    renderColoredCutoutModel(this.getParentModel(), CARTOGRAPHER_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "cleric" ->
                    renderColoredCutoutModel(this.getParentModel(), CLERIC_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "farmer" ->
                    renderColoredCutoutModel(this.getParentModel(), FARMER_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "fisherman" ->
                    renderColoredCutoutModel(this.getParentModel(), FISHERMAN_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "fletcher" ->
                    renderColoredCutoutModel(this.getParentModel(), FLETCHER_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "leatherworker" ->
                    renderColoredCutoutModel(this.getParentModel(), LEATHERWORKER_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "librarian" ->
                    renderColoredCutoutModel(this.getParentModel(), LIBRARIAN_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "mason" ->
                    renderColoredCutoutModel(this.getParentModel(), MASON_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "nitwit" ->
                    renderColoredCutoutModel(this.getParentModel(), NITWIT_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "shepherd" ->
                    renderColoredCutoutModel(this.getParentModel(), SHEPHERD_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "toolsmith" ->
                    renderColoredCutoutModel(this.getParentModel(), TOOLSMITH_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case "weaponsmith" ->
                    renderColoredCutoutModel(this.getParentModel(), WEAPONSMITH_LOCATION.getModel(), poseStack, source, i, entityRenderState, -1);
            case null, default -> {
            }
        }
        poseStack.popPose();
