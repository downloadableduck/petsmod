...
public class ClientGoatModel extends QuadrupedModel<ClientGoat> {
    ...
    public void setupAnim(ClientGoat goat, float f, float g, float h, float i, float j) {
        super.setupAnim(goat, f, g, h, i, j);
        if (CONFIG.isBaby) {
            this.head.getChild("left_horn").visible = false;
            this.head.getChild("right_horn").visible = false;
        }
    }
}
