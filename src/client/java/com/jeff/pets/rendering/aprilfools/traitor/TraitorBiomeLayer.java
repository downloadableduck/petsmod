...
public class TraitorBiomeLayer extends RenderLayer<@NotNull Traitor, @NotNull ClientEvokerModel<Traitor>> {
    ...
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int i, Traitor entityRenderState, float f, float g, float h, float j, float k, float m) {
        ...
        switch (CONFIG.traitorSkin) {
            case "desert" ->
                    renderColoredCutoutModel(this.getParentModel(), DESERT_LOCATION.getModel(), poseStack, bufferSource, i, entityRenderState, -1);
            case "jungle" ->
                    renderColoredCutoutModel(this.getParentModel(), JUNGLE_LOCATION.getModel(), poseStack, bufferSource, i, entityRenderState, -1);
            case "savanna" ->
                    renderColoredCutoutModel(this.getParentModel(), SAVANNA_LOCATION.getModel(), poseStack, bufferSource, i, entityRenderState, -1);
            case "snow" ->
                    renderColoredCutoutModel(this.getParentModel(), SNOW_LOCATION.getModel(), poseStack, bufferSource, i, entityRenderState, -1);
            case "swamp" ->
                    renderColoredCutoutModel(this.getParentModel(), SWAMP_LOCATION.getModel(), poseStack, bufferSource, i, entityRenderState, -1);
            case "taiga" ->
                    renderColoredCutoutModel(this.getParentModel(), TAIGA_LOCATION.getModel(), poseStack, bufferSource, i, entityRenderState, -1);
            case "plains" ->
                    renderColoredCutoutModel(this.getParentModel(), PLAINS_LOCATION.getModel(), poseStack, bufferSource, i, entityRenderState, -1);
