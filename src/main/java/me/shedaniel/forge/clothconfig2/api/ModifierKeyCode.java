    ...
    default boolean matchesCurrentMouse() {
        if (!isUnknown() && getType() == InputMappings.Type.MOUSE && getModifier().matchesCurrent()) {
            switch (getKeyCode().getKeyCode()) {
                case 0:
                    return Minecraft.getMinecraft().mouseHelper.isLeftDown();
                case 1:
                    return Minecraft.getMinecraft().mouseHelper.isRightDown();
                case 2:
                    return Minecraft.getMinecraft().mouseHelper.isMouseGrabbed();
            }
        }
        return false;
    }
