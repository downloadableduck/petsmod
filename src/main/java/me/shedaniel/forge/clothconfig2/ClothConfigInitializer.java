...
public class ClothConfigInitializer {
    ...
    public ClothConfigInitializer() {
        ...
        ModLoadingContext.get().registerExtensionPoint(ExtensionPoint.CONFIGGUIFACTORY, () -> (client, parent) -> {
            return new PetsConfigScreen<>().build();
        });
    }
    ...
    public static ConfigBuilder getConfigBuilder() {
        ...
        scrolling.addEntry(new TooltipListEntry<Object>(I18n.get("option.cloth-config.setDefaultSmoothScroll"), null) {
            final int width = 220;
        ...
        scrolling.addEntry(new TooltipListEntry<Object>(I18n.get("option.cloth-config.disableSmoothScroll"), null) {
            final int width = 220;
            private final Widget buttonWidget = new AbstractButton(0, 0, 0, 20, getFieldName()) {
                @Override
                public void onPress() {
...
//        testing.addEntry(entryBuilder.startDropdownMenu("lol apple", DropdownMenuBuilder.TopCellElementBuilder.ofItemObject(Items.APPLE), DropdownMenuBuilder.CellCreatorBuilder.ofItemObject()).setDefaultValue(Items.APPLE).setSelections(Registry.ITEM.stream().sorted(Comparator.comparing(Item::toString)).collect(Collectors.toCollection(LinkedHashSet::new))).setSaveConsumer(item -> System.out.println("save this " + item)).build());
        testing.addEntry(entryBuilder.startKeyCodeField("Cool Key", InputMappings.UNKNOWN).setDefaultValue(InputMappings.UNKNOWN).build());
        testing.addEntry(entryBuilder.startModifierKeyCodeField("Cool Modifier Key", ModifierKeyCode.of(InputMappings.Type.KEYSYM.getOrCreate(79), Modifier.of(false, true, false))).setDefaultValue(ModifierKeyCode.of(InputMappings.Type.KEYSYM.getOrCreate(79), Modifier.of(false, true, false))).build());
