...
public abstract class ClothConfigScreen extends GuiScreen {
    ...
    public void updateScreen() {
        super.updateScreen();
        for (IGuiEventListener child : eventListeners)
            if (child instanceof ITickable)
                ((ITickable) child).tick();
            else if (child instanceof GuiTextField)
                ((GuiTextField) child).updateCursorCounter();
    }
    ...
    protected void initGui() {
        ...
        eventListeners.add(listWidget = new ListWidget(minecraft, width, height, isShowingTabs() ? 70 : 30, height - 32, getBackgroundLocation()));
        ...
        addButton(quitButton = new GuiButton(new Random().nextInt(), 25, height - 26, buttonWidths, 20, edited ? I18n.format("text.cloth-config.cancel_discard") : I18n.format("gui.cancel")) {
            @Override
        ...
        addButton(saveButton = new GuiButton(new Random().nextInt(), 25 + 6 + buttonWidths, height - 26, buttonWidths, 20, "") {
            @Override
        ...
        addButton(applyButton = new GuiButton(new Random().nextInt(), 25 + (6 + buttonWidths) * 2, height - 26, buttonWidths, 20, I18n.format("text.cloth-config.apply")) {
            @Override
            public void mousePressed(double d, double u) {
                saveAll(true);
                super.mousePressed(d, u);
        ...
        if (isShowingTabs()) {
            ...
            eventListeners.add(buttonLeftTab = new GuiButton(new Random().nextInt(), 4, 44, 12, 18, "") {
                @Override
                public void mousePressed(double d, double u) {
                ...
                public void drawButton(int int_1, int int_2, float float_1) {
                    ...
                    int int_3 = this.getHoverState(this.hovered);
            ...
            eventListeners.addAll(tabButtons);
            eventListeners.add(buttonRightTab = new GuiButton(new Random().nextInt(), width - 16, 44, 12, 18, "") {
                @Override
    ...
    public boolean mouseScrolled(double amount) {
        ...
        double mouseX = client.mouseHelper.getMouseX();
        double mouseY = client.mouseHelper.getMouseY();
    ...
    public void drawButton(int int_1, int int_2, float float_1) {
        ...
        if (isShowingTabs()) {
            ...
            tabButtons.forEach(widget -> widget.drawButton(int_1, int_2, float_1));
            drawTabsShades(0, isTransparentBackground() ? 120 : 255);
            ScissorsHandler.INSTANCE.removeLastScissor();
            buttonLeftTab.drawButton(int_1, int_2, float_1);
            buttonRightTab.drawButton(int_1, int_2, float_1);
        } else
    ...
    private class QuitSaveConsumer implements BooleanConsumer, GuiYesNoCallback {
        @Override
        ...
        public void confirmResult(boolean p_confirmResult_1_, int p_confirmResult_2_) {
            this.accept(p_confirmResult_1_);
        }
    }
    ...
    public class ListWidget<R extends DynamicElementListWidget.ElementEntry<R>> extends DynamicElementListWidget<R> {
        ...
        @Override
        protected List<? extends IGuiEventListener> getEventListeners() {
