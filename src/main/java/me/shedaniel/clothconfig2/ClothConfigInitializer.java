...
public class ClothConfigInitializer implements ClientModInitializer {
        ...
        if (FabricLoader.getInstance().isModLoaded("modmenu")) {
            try {
                ...
                method.invoke(null, "cloth-config2", (Runnable) () -> {
                    try {
                        MinecraftClient.getInstance().setScreen(getConfigBuilderWithDemo().build());
    ...
    public static ConfigBuilder getConfigBuilder() {
        ConfigBuilder builder = ConfigBuilder.create().setParentScreen(MinecraftClient.getInstance().currentScreen).setTitle("title.cloth-config2.config");
        ...
        scrolling.addEntry(new TooltipListEntry<Object>(I18n.translate("option.cloth-config2.setDefaultSmoothScroll"), null, false) {
            ...
            public void render(int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isSelected, float delta) {
                ...
                this.buttonWidget.method_891(mouseX, mouseY, delta);
            }
        });
...
//        testing.addEntry(entryBuilder.startDropdownMenu("lol apple", DropdownMenuBuilder.TopCellElementBuilder.ofItemObject(Items.APPLE), DropdownMenuBuilder.CellCreatorBuilder.ofItemObject()).setDefaultValue(Items.APPLE).setSelections(Registry.ITEM.stream().sorted(Comparator.comparing(Item::toString)).collect(Collectors.toCollection(LinkedHashSet::new))).setSaveConsumer(item -> System.out.println("save this " + item)).build());
        testing.addEntry(entryBuilder.startKeyCodeField("Cool Key", class_4107.field_19910).setDefaultValue(class_4107.field_19910).build());
        testing.addEntry(entryBuilder.startModifierKeyCodeField("Cool Modifier Key", ModifierKeyCode.of(class_4107.class_4109.KEYSYM.method_18162(79), Modifier.of(false, true, false))).setDefaultValue(ModifierKeyCode.of(class_4107.class_4109.KEYSYM.method_18162(79), Modifier.of(false, true, false))).build());
