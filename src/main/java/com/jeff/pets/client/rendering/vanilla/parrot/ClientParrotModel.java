...
public class ClientParrotModel extends EntityModel<ClientParrot> {
    ...
    @Override
    public void setAngles(ClientParrot parrot, float i, float f, float g, float h, float j, float k) {
        ParrotEntityModel.Pose pose = parrot.isPassenger() ? ParrotEntityModel.Pose.ON_SHOULDER : ParrotEntityModel.Pose.FLYING;
        this.field_3452.pitch = k * ((float) Math.PI / 180F);
        this.field_3452.yaw = j * ((float) Math.PI / 180F);
        ...
        switch (pose) {
            ...
            case PARTY:
                ...
                break;
            case STANDING:
                Cuboid var10000 = this.field_3450;
                var10000.pitch += MathHelper.cos(f * 0.6662F) * 1.4F * g;
                var10000 = this.field_3457;
                var10000.pitch += MathHelper.cos(f * 0.6662F + (float) Math.PI) * 1.4F * g;
            case FLYING:
            case ON_SHOULDER:
            default:
                float n = h * 0.3F;
    ...
    private void prepare(ParrotEntityModel.Pose pose) {
