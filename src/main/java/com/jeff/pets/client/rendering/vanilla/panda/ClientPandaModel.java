...
public class ClientPandaModel extends QuadrupedModel<ClientPanda> {
    ...
    public ClientPandaModel(ModelPart modelPart) {
    ...
    public void prepareMobModel(ClientPanda panda, float f, float g, float h) {
    ...
    public void setupAnim(ClientPanda panda, float f, float g, float h, float i, float j) {
        ...
        if (bl2) {
            if (k < 15) {
                this.head.xRot = (-(float) Math.PI / 4F) * (float) k / 14.0F;
            } else if (k < 20) {
                float l = (float) ((k - 15) / 5);
                this.head.xRot = (-(float) Math.PI / 4F) + ((float) Math.PI / 4F) * l;
            }
        }
