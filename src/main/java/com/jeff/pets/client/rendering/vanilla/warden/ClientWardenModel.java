...
public class ClientWardenModel extends HierarchicalModel<ClientWarden> {
    ...
    public ClientWardenModel(ModelPart modelPart) {
    ...
    @Override
    public void setupAnim(ClientWarden warden, float f, float g, float h, float i, float j) {
    ...
    private void animateWalk(float f, float g) {
        ...
        var10000 = this.head;
        var10000.xRot += 1.2F * Mth.cos(i + ((float) Math.PI / 2F)) * l;
    ...
    private void animateTendrils(ClientWarden warden, float f, float g) {
        float h = 1 * (float) (Math.cos((double) f * (double) 2.25F) * Math.PI * (double) 0.1F);
