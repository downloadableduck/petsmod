...
public class ClientSheep extends TamableAnimal {
    private int eatAnimationTick;

    public ClientSheep(EntityType<? extends @NotNull TamableAnimal> entityType, Level level) {
    ...
    public float getHeadEatAngleScale(float f) {
        if (this.eatAnimationTick > 4 && this.eatAnimationTick <= 36) {
            ...
            return ((float) Math.PI / 5F) + 0.21991149F * Mth.sin(g * 28.7F);
        } else {
    ...
    public void tick() {
        ...
        if (ambient == 1) {
            level().playLocalSound(this, SoundEvents.SHEEP_AMBIENT, SoundSource.AMBIENT, 1.0f, 1.0f);
        }
    }
