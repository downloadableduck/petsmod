package com.jeff.pets.vanilla.hostile;
...
public class ClientCreaking extends TamableAnimal{
    public ClientCreaking(EntityType<? extends @NotNull TamableAnimal> entityType, Level level) {
    ...
    public void tick() {
        ...
        if (this.walkAnimation.isMoving()) {
            level().playLocalSound(this, SoundEvents.CREAKING_STEP, SoundSource.AMBIENT, 1.0f, 1.0f);
        }

        int ambient = (int) (Math.random() * (60 * 20));
        if (ambient == 1) {
            level().playLocalSound(this, SoundEvents.CREAKING_AMBIENT, SoundSource.AMBIENT, 1.0f, 1.0f);
        }
    }
